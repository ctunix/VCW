...
- ボーカルと伴奏の分離バックエンドを UVR5 から PyMSS に変更し、推論中の分離処理を途中で中断できるようにしました（より優れたボーカル分離アルゴリズムにより、推論品質を大幅に向上できます）。
- 入力音声の読み込みとリサンプリング処理を最適化し、可能な場合は GPU で処理することで、推論効率を向上させ、CPU 使用率を低減しました。
- リアルタイム声変換で入力ノイズ除去または出力ノイズ除去を有効にした際に発生する CUDA Graph エラーを修正しました。
- Windows 環境における学習プロセスの libuv 互換性問題を修正しました。
...
- 現在最も強力なオープンソースの人間の声のピッチ抽出モデル RMVPE をトレーニングし、VCW のトレーニング、オフライン/リアルタイム推論に使用。pytorch/onnx/DirectML をサポート
...
- ~~crepe の精度サポートと VCW-config の同期：既に RMVPE に置き換えられているため不要。これをサポートするには torchcrepe ライブラリも同期する必要があり、面倒~~
- F0 エディタとの連携
