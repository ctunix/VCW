...
### pretrained modelの指定
VCWではモデルの訓練を0からではなく、事前学習済みの重みから開始するため、少ないデータセットで学習を行えます。

デフォルトでは

- 音高ガイドを考慮する場合、`VCWのある場所/pretrained/f0G40k.pth`と`VCWのある場所/pretrained/f0D40k.pth`を読み込みます。
- 音高ガイドを考慮しない場合、`VCWのある場所/pretrained/G40k.pth`と`VCWのある場所/pretrained/D40k.pth`を読み込みます。
...
### indexの学習
VCWでは学習時に使われたHuBERTの特徴量を保存し、推論時は学習時の特徴量から近い特徴量を探してきて推論を行います。この検索を高速に行うために事前にindexの学習を行います。
