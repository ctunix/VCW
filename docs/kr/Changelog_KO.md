...
- 보컬 및 반주 분리 백엔드를 UVR5에서 PyMSS로 변경하고, 추론 중 분리 작업을 중간에 중단할 수 있도록 했습니다(더 나은 보컬 분리 알고리즘은 추론 품질을 크게 향상시킬 수 있습니다).
- 입력 오디오 로딩 및 리샘플링 과정을 최적화하여 가능한 경우 GPU에서 처리함으로써 추론 효율을 높이고 CPU 사용량을 줄였습니다.
- 실시간 음성 변환에서 입력 노이즈 제거 또는 출력 노이즈 제거를 활성화할 때 발생하던 CUDA Graph 오류를 수정했습니다.
- Windows 환경의 학습 프로세스에서 발생하던 libuv 호환성 문제를 수정했습니다.
...
- 현재 가장 강력한 오픈 소스 보컬 피치 추출 모델 RMVPE를 훈련하고, 이를 VCW 훈련, 오프라인/실시간 추론에 사용하며, PyTorch/Onx/DirectML을 지원합니다.
...
- 탭 페이지에 "자주 묻는 질문과 답변" 추가. (github VCW wiki 참조 가능)
