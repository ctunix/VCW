...
def load_hubert_model(device, is_half=False):
    """Load the local Transformers HuBERT/ContentVec model for VCW."""
...
def extract_hubert_features(model, source, version, padding_mask=None):
    """Return the VCW v1 (256-D) or v2 (768-D) HuBERT representation.

    Transformers hidden_states[N] is numerically equivalent to the source checkpoint's
    output_layer=N for this converted checkpoint. VCW v1 uses layer 9 followed
    by final_proj; VCW v2 uses the final (12th) encoder layer directly.
    """
    if version not in {"v1", "v2"}:
        raise ValueError(f"Unsupported VCW feature version: {version!r}")
